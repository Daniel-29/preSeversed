...
const register = {
    ...
    async resolve(parent, args, context) {
        ...
        let emailIsRegister = await User.findOne({ email: email });
        if (emailIsRegister) {
            throw new Error('email not available!')
        }
        let usernameIsRegister = await User.findOne({ username: args.username });
        if (usernameIsRegister) {
...
const updateUser = {
    ...
    async resolve(parent, args, context) {
        ...
        const email = args.email.toLowerCase();
        let userData = await User.findOne({ _id: args.id });
